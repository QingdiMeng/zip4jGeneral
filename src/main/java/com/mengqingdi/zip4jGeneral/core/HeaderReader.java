...
import com.mengqingdi.zip4jGeneral.model.*;
...
public class HeaderReader {
    ...
    private DataInputStream zip4jDis = null;
    private ZipModel zipModel;

    public HeaderReader(DataInputStream zip4jdis) {
        this.zip4jDis = zip4jdis;
    }
    ...
    private EndCentralDirRecord readEndOfCentralDirectoryRecord() throws ZipException {
        ...
        if (zip4jDis == null) {
            throw new ZipException("data input stream was null", ZipExceptionConstants.dataInputStreamNull);
        }
        ...
        try {
            ...
            byte[] ebs = new byte[4];
            zip4jDis.reset();
            int pos = zip4jDis.available() - InternalZipConstants.ENDHDR;
            ...
            do {
                ...
                zip4jDis.skip(pos--);
                counter++;
            } while ((Raw.readLeInt(zip4jDis, ebs) != InternalZipConstants.ENDSIG) && counter <= 3000);
            ...
            //number of this disk
            readIntoBuff(zip4jDis, shortBuff);
            ...
            if (commentLength > 0) {
                ...
                readIntoBuff(zip4jDis, commentBuf);
    ...
    private CentralDirectory readCentralDirectory() throws ZipException {
        ...
        if (zip4jDis == null) {
            throw new ZipException("random access file was null", ZipExceptionConstants.dataInputStreamNull);
        }
        ...
        try {
            ...
            zip4jDis.reset();
            zip4jDis.skip(offSetStartCentralDir);
            ...
            for (int i = 0; i < centralDirEntryCount; i++) {
                ...
                if (fileNameLength > 0) {
                    ...
                    readIntoBuff(zip4jDis, fileNameBuf);
...
//				}
                ...
                if (fileCommentLength > 0) {
                    ...
                    readIntoBuff(zip4jDis, fileCommentBuf);
            ...
            if (sizeOfData > 0) {
                ...
                readIntoBuff(zip4jDis, sigDataBuf);
    ...
    private Zip64EndCentralDirLocator readZip64EndCentralDirLocator() throws ZipException {
        ...
        try {
            ...
            readIntoBuff(zip4jDis, longBuff);
    ...
    private Zip64EndCentralDirRecord readZip64EndCentralDirRec() throws ZipException {
        ...
        try {
            zip4jDis.reset();
            zip4jDis.skip(offSetStartOfZip64CentralDir);
            ...
            if (extDataSecSize > 0) {
                ...
                readIntoBuff(zip4jDis, extDataSecRecBuf);
    ...
    private void setFilePointerToReadZip64EndCentralDirLoc() throws ZipException {
        try {
            ...
            do {
                zip4jDis.reset();
                zip4jDis.skip(pos--);
            } while (Raw.readLeInt(zip4jDis, ebs) != InternalZipConstants.ENDSIG);

            pos = pos + 4;
            zip4jDis.reset();
            zip4jDis.skip(pos - 4 - 4 - 8 - 4 - 4);
        } catch (IOException e) {
            throw new ZipException(e);
        }
    }
    ...
    public LocalFileHeader readLocalFileHeader(FileHeader fileHeader) throws ZipException {
        if (fileHeader == null || zip4jDis == null) {
...
//								- ZipConstants.STD_DEC_HDR_SIZE);
    ...
    private byte[] readIntoBuff(DataInputStream dis, byte[] buf) throws ZipException {
        try {
            if (dis.read(buf, 0, buf.length) != -1) {
                return buf;
            } else {
